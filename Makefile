image:
	docker build -t bbaerthlein/docker-ddns:latest .

console:
	docker run -it -p 8080:8080 -p 53:53 -p 53:53/udp --rm davd/docker-ddns:latest bash

devconsole:
	docker run -it --rm -v ${PWD}/dyndns:/usr/src/app -w /usr/src/app golang:latest bash

server_test:
	docker run -it -p 8080:8080 -p 53:53 -p 53:53/udp --env-file envfile --rm davd/docker-ddns:latest

unit_tests:
	docker run -it --rm -v ${PWD}/dyndns:/go/src/dyndns -w /go/src/dyndns golang:latest /bin/bash -c "go get && go test -v"
...
deploy: image
	docker run -it -d -p 8080:8080 -p 53:53 -p 53:53/udp --env-file envfile --name=dyndns bbaerthlein/docker-ddns:latest
